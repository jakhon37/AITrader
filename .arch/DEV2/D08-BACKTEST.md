...
- D02-DATA: DataStore.get_ohlcv(), get_economic_events() (historical reads only)
- D04-TECHNICAL: imports TechnicalEngine directly (local instance, not via bus)
- D05-DECISION: imports DecisionEngine directly (local instance)
- D06-EXECUTION: imports SimBroker + ExecutionEngine directly (never live broker)

D08 creates private instances of D04/D05/D06 wired to an ISOLATED in-process bus.
...
3. Instantiate D04 TechnicalEngine, D05 DecisionEngine, D06 SimBroker + ExecutionEngine
   all wired to the REPLAY bus and ReplayClock (not production bus/clock)
...
10. Milestone: strategy replay runs at 10x from CLI
