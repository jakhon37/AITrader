...
from src.data.feeds.lock import DUKASCOPY_LOCK
...
class DukascopyFeed(OHLCVFeed):
    ...
    def _download_day(self, symbol: str, year: int, month: int, day: int) -> Optional[pd.DataFrame]:
        url = (
        ...
        for attempt in range(retries):
            try:
                res = requests.get(url, timeout=self._request_timeout_sec)
                if res.status_code in (403, 429):
                    backoff = [60, 120, 300][attempt]
                    _log.warning(
                ...
                if attempt == retries - 1:
                    raise DataError(f"Dukascopy connection error on {url}: {exc}") from exc
                time.sleep(10)
    ...
    def _fetch_today_m1_from_ticks(self, symbol: str) -> pd.DataFrame:
        ...
        def _dl(year: int, month: int, day: int, hour: int) -> Optional[pd.DataFrame]:
            return self._download_hour_ticks(symbol, year, month, day, hour)

        m1 = fetch_today_m1_from_ticks(_dl)
        if not m1.empty:
            _log.info(
    ...
    def _fetch_m1_hybrid(
        ...
        while current < today and current <= end_date:
            day_df = self._download_day(symbol, current.year, current.month, current.day)
