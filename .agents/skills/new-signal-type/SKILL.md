...
Safely adds a new signal type or extends an existing signal model in Division 1 (D01-CORE) (`src/core/contracts.py`). This is the highest-impact change in the codebase — every division depends on these contracts.
...
4. **Does Division 7 (D07-NOTIFIER) (notifications) format this type?**
   - If yes: add a formatter in `src/notifier/router.py`

5. **Does Division 10 (UI) display this?**
   - If yes: update TypeScript types in `frontend/src/types.ts`
...
Find where the signal is constructed (e.g. `src/technical/engine.py` for `TechnicalSignal`) and populate the new field.

### Step 3 — Update Division 7 (D07-NOTIFIER) formatter if signal is displayed

```python
# src/notifier/router.py
...
```python
# src/core/__init__.py — add to exports
...
await self.bus.publish(MyNewSignal(
    signal_id=new_signal_id("MN", instrument.value, self.clock.now()),
...
### Step 6 — Add formatter in Division 7 (D07-NOTIFIER)
...
And update the WebSocket message handler in `frontend/src/hooks/useWebSocket.ts` to handle `"MyNewSignal"` type.
