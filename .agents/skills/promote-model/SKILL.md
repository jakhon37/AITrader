...
PYTHONPATH=src CONFIG_DIR=$(pwd)/config python scripts/project_overview.py --status staging
```
...
```bash
PYTHONPATH=src CONFIG_DIR=$(pwd)/config python scripts/project_overview.py --status prod
```
...
**System checks**
- [ ] Model loads without error: `python scripts/project_overview.py --validate --model-id {id}`
- [ ] Decision engine picks up new model: check `/health` after promotion
- [ ] Feature columns in model metadata match current Division 1 (D01-CORE) (D04-TECHNICAL) indicator output
...
python scripts/project_overview.py --model-id bad_model_id --to staging

# 2. Promote the previous prod model back
python scripts/project_overview.py --model-id previous_good_model_id --to prod
