...
PYTHONPATH=src CONFIG_DIR=$(pwd)/config python scripts/validate_data.py

# Validate a specific instrument
PYTHONPATH=src CONFIG_DIR=$(pwd)/config python scripts/validate_data.py --instrument EURUSD

# Validate specific instrument + timeframe
PYTHONPATH=src CONFIG_DIR=$(pwd)/config python scripts/validate_data.py --instrument XAUUSD --timeframe 1h
```
