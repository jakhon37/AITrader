...
## Step 1 — Add to the Instrument enum (Division 4)
...
```bash
PYTHONPATH=src CONFIG_DIR=$(pwd)/config python scripts/validate_data.py --instrument NEWPAIR
```

## Step 4 — Add to fundamental impact map (Division 2)

```python
# src/fundamental/impact_map.py
...
The default `TF_WEIGHTS` in Division 3 are instrument-agnostic, which is fine for Forex. If the new instrument has different characteristics (e.g. crypto trades 24/7, oil has specific session patterns), you may need instrument-specific regime thresholds in `src/features/regime.py`:

```python
# src/features/regime.py — instrument-specific volatility thresholds
...
## Step 6 — Update Telegram formatter (Division 7)
...
// frontend/src/components/panels/InstrumentSelector.tsx
