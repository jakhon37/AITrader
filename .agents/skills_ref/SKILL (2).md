...
```
Step 1:  Division 4   ← MUST be first. Blocks everything.
Step 2:  Division 1   ← Blocks Division 2, 3, 8
Step 3a: Division 3   ← Can parallel with 3b
Step 3b: Division 2   ← Can parallel with 3a
...
Step 6a: Division 7   ← Can start after Div 4
...
```
Is Division 4 COMPLETE?
  No → Work on Division 4. Nothing else can proceed.
  Yes → Continue...
...
Is Division 6 COMPLETE?
  ...
  Yes → Everything else is parallel. Pick by priority:
    ...
    - Division 7 (Telegram) — simplest, highest daily value
...
|---|---|---|
| Can't import from `signals.contracts` | Division 4 not installed | `pip install -e ".[dev]"` from project root |
...
# Or for blocked:
sed -i 's/^Status: PLANNED/Status: BLOCKED_BY: Division 4/' DIVISION_5.md
```
