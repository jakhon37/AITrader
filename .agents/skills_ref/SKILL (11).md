---
name: add-indicator
description: Use this skill when the user wants to add a new technical indicator, extend the indicator set, add a new TA signal component, or wire a new indicator into the confluence voting system in Division 3.
---

# add-indicator

Adds a new technical indicator to Division 3 (`src/features/indicators.py`) and wires it into the `TimeframeRunner` voting system.

## Step 1 — Implement the indicator function

Open `src/features/indicators.py`. Add a new function following this exact pattern:
...
In `src/features/indicators.py`, find the `compute_all_indicators(df)` function and add your new function to the list:
...
## Step 3 — Add a vote in TimeframeRunner

Open `src/features/timeframe_runner.py`. In the `_vote()` method, add a vote block for your indicator:
...
Create or extend `tests/unit/test_indicators.py`:
...
Run the TimeframeRunner unit test to confirm the new vote participates in direction computation:

```bash
PYTHONPATH=src CONFIG_DIR=$(pwd)/config pytest tests/unit/test_timeframe_runner.py -v
```
...
- **Never** modify `contracts.py` or any Division 4 file — indicators are internal to Division 3
