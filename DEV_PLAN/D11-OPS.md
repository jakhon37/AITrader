...
## Emits
Bus topic: `BusChannel.SYSTEM_HEALTH` — `SystemHealthEvent` per CONTRACTS.md, published
on any threshold breach. D07 already subscribes to this channel and forwards to Telegram.
D11 never calls D07 directly — same bus-only rule as every other producer/subscriber pair.
